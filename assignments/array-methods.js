...
let fullName = [];
runners.forEach(a => fullName.push(`${a.first_name} ${a.last_name}`))
...
// let largeShirts = [];
let largeShirts = runners.filter(a => a.shirt_size === "L");
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce((acc, a) => {return acc + a["donation"]}, 0);
...
// Problem 1
let emails = runners.map(p => p.email);
console.log(emails);
...
let lambdaschool = runners.filter(function(amount){
